...
//mono voice MIDI chan 1 1=gate 2 = pitch 3 = velocity 4 = CC 1
//+ 4x drums with velocity CVs MIDI chan 2 5="c" drum 6 = "c" drum velocity etc. drums repeat over keyboard.
...
void BothNoteOff(byte channel, byte pitch, byte velocity) { //this is called by handle noteoff and note on when velocity = 0
  ...
  if (channel == 2) { //output drum trigs and velocity from chan 2
    digitalWriteFast(out2pin[((pitch % 4) << 1) + 4], LOW);
    //analogWrite(out2pin[((pitch%4)<<1)+5],0);  uncomment if you want the drum trigger velocity outs to turn off at end of note
  }
}
...
void HandleNoteOn(byte channel, byte pitch, byte velocity) {
  ...
  if (velocity != 0) {
    ...
    if (channel == 2) { //output drum trigs and velocity from chan 2
      digitalWriteFast(out2pin[((pitch % 4) << 1) + 4], HIGH);
      analogWrite(out2pin[((pitch % 4) << 1) + 5], velocity<<5 );
    }
  }
